...
def cleanse_kym(path):
    ...
    for meme in memes:
        ...
        doc = nlp(meme_dict['description'] if meme_dict['description'] else '' + meme_dict['about'] if meme_dict[
            'about'] else '')
        if doc:
            sentiments = []
    ...
    df = df.rename(columns={'tags_normalized': 'tags'})
    df.to_parquet("/opt/airflow/dags/kym_processed.parquet")
...
def cleanse_vision(path):
    ...
    df['annot_array'] = df['annot_array'].astype("string").str.replace("'", ",").str.replace(", ,",",").str.strip("[,").str.strip("]")
...
def cleanse_spotlight(path):
    ...
    df['types_DBpedia'] = df['types_DBpedia'].astype("string").str.replace("'", ",").str.replace(", ,",",").str.strip("[,").str.strip("]")
    df['types_Schema'] = df['types_Schema'].astype("string").str.replace("'", ",").str.replace(", ,",",").str.strip("[,").str.strip("]")
...
def merge_all(path1, path2, path3):
    ...
    final[["title", "description", "origin", "type", "about", "origin_references", "spread_references", "search_keywords","tags", "webDetection_bestGuessLabel", "@text"]] = final[["title", "description", "origin", "type", "about", "origin_references", "spread_references", "search_keywords","tags", "webDetection_bestGuessLabel", "@text"]].replace({"'": "''"}, regex=True)
    final[["annot_array", "types_DBpedia", "types_Schema"]] = final[["annot_array", "types_DBpedia", "types_Schema"]].replace({"'": "''"})
...
def create_fact_table_query(path):
    ...
    with open("/opt/airflow/dags/fact_table_inserts.sql", "w") as f:
        ...
        f.write(
            ...
            "count INT,\n"
            "PRIMARY KEY (date_id, origin_id, type_id, tag_id));"
        )
        for index, row in df_iterable:
            date_id = row['date_id']
            origin_id = row['origin_id']
            ...
            f.write(
                ...
                ") ON CONFLICT (date_id, origin_id, type_id, tag_id) DO NOTHING;\n"
            )
...
def create_graph_table_query(path):
    ...
    with open("/opt/airflow/dags/graph_table_inserts.sql", "w") as f:
        ...
        for index, row in df_iterable:
            url = row['url']
            title = re.sub('[\W_]+', '', row['title'])
            if title[0].isdigit():
                title = "x" + title
            row_title = row['title'].replace('"','\\"')
            f.write(
                f"CREATE ({title}:meme{{title:\"{row_title}\", url:\"{url}\"}})\n"
            )
        df_iterable = data.iterrows()
        for index, row in df_iterable:
            title = re.sub('[\W_]+', '', row['title'])
            ...
            if not pd.isna(row["parent"]):
                ...
                if not parent.empty:
                    parent_title = re.sub('[\W_]+', '', parent.to_string(index = False))
                    if parent_title[0].isdigit():
            ...
            if not pd.isna(row["siblings"]):
                ...
                for sibling in siblings:
                    ...
                    if not sib.empty:
                        sib_title = re.sub('[\W_]+', '', sib.to_string(index = False))
                        if sib_title[0].isdigit():
                            sib_title = "x" + sib_title
                        f.write(
            ...
            if not pd.isna(row["children"]):
                ...
                for child in children:
                    ...
                    if not chi.empty:
                        chi_title = re.sub('[\W_]+', '', chi.to_string(index = False))
                        if chi_title[0].isdigit():
                            chi_title = "x" + chi_title
                        f.write(
...
task_five >> task_six >> [task_seven_a, task_seven_b, task_seven_c, task_seven_d, task_seven_e, task_seven_f, task_seven_i]
